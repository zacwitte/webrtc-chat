(function (window, PUBNUB) {
  ...
  function extendAPI(PUBNUB, uuid) {
    ...
    function SignalingChannel(pubnub, selfUuid, otherUuid) {
      ...
      this.send = function (message, force) {
        ...
        if (this.peerReady === true || force === true) {
          ...
          console.log("Sending to: ", PREFIX + otherUuid, " message ", message);
          pubnub.publish({
    ...
    function personalChannelCallback(message) {
      console.log("Personal channel callback: ", message);
      ...
      if (message.uuid != null) {
        ...
        if (message.sdp != null) {
          console.log("Remote Session Description:", message.sdp);
          connection.connection.setRemoteDescription(new RTCSessionDescription(message.sdp), function () {
            ...
            console.log("Adding ice candidates", candidates, connection);
            for (var i = 0; i < candidates.length; i++) {
              console.log("Remote ICE Candidate (backfill):", candidates[i]);
            ...
            if (connection.connection.signalingState === 'have-remote-offer') {
              console.log("Creating answer...", message.uuid);
    ...
    PUBNUB.subscribe({
      ...
      connect: function () {
        ...
        console.log("Connected to channel: ", PREFIX + uuid);
        ...
        for (var i = 0; i < CONNECTION_QUEUE.length; i++) {
          ...
          if (args.length > 1) {
            ...
            console.log("Connection Queue Description", args);
    ...
    API['gotDescription'] = function (description, connection) {
      ...
      if (connection.connection.signalingState !== 'have-local-offer') {
        console.log("Local Session Description", description.sdp);
    ...
    API['createP2PConnection'] = function (uuid, offer, callback) {
      if (PEER_CONNECTIONS[uuid] == null) {
        ...
        var onDataChannelCreated = function (event) {
          ...
          console.log("Add handler for streams.");
          PEER_CONNECTIONS[uuid].connection.onaddstream = function (event) {
            console.log("On Stream Add", event, PEER_CONNECTIONS[uuid].stream);
        ...
        var iceCandidateBufferInterval = setInterval(flushIceCandidateBuffer, 100);
        ...
        if (UUID > uuid) {
          ...
          console.log("Creating offer...", uuid);
        ...
        console.log("Trying to connect to already connected user: " + uuid);
      }
    };

    // Helper function for sending messages with different types.
    function handleMessage(connection, message) {
      console.log("Handling message", connection, message);
      if (message.type === PUBLISH_TYPE.STREAM) {
        console.log("Adding stream", message.stream);
    ...
    API['_peerPublish'] = function (uuid) {
      if (PUBLISH_QUEUE[uuid] && PUBLISH_QUEUE[uuid].length > 0) {
        console.log("Connected", PEER_CONNECTIONS[uuid].connected, uuid);
          ...
          if (PEER_CONNECTIONS[options.user] == null) {
            PUBNUB.createP2PConnection(options.user, null, function () {
              if (options.stream != null) {
                console.log("Publishing stream to user", options.stream, options.user);
    ...
    API['subscribe'] = (function (_super) {
      return function (options) {
        ...
        if (options.user != null) {
          ...
          console.log("Subscribing to user: ", options.user, connection.history);
    ...
    API['peerConnection'] = function (uuid, callback) {
      if (callback) {
        ...
        console.log("PUBNUB.peerConnection should be called with a callback");
      }
    };
    ...
    API['dataChannel'] = function (uuid, callback) {
      if (callback) {
        ...
        console.log("PUBNUB.dataChannel should be called with a callback");
      }
    };
  ...
  PUBNUB['init'] = (function (_super) {
    return function (options) {
      ...
      console.log("PubNub init: ", options);
